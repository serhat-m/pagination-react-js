...
const Pagination = ({ entriesPerPage, totalEntries, currentPage, offset, classNames, showFirstNumberAlways, showLastNumberAlways, navStart, navEnd, navPrev, navNext, navPrevCustom, navNextCustom }: Pagination) => {
    function generatePagination(pageNumbers: number[], currentPage: Pagination["currentPage"], offset?: number) {
        if (offset === undefined) {
            return pageNumbers.map(pageNumber => {
                return <PaginationItem key={pageNumber} onClick={() => currentPage.set(pageNumber)} className={classNames?.item} activeClassName={classNames?.itemActive} active={pageNumber === currentPage.get}>{pageNumber}</PaginationItem>
            })
        } else {
            ...
            return <Fragment>
                {
                    firstNumber && lastNumber
                        ?
                        <Fragment>
                            {showFirstNumberAlways && <PaginationItem key={firstNumber} onClick={() => currentPage.set(firstNumber)} className={classNames?.item} activeClassName={classNames?.itemActive} active={firstNumber === currentPage.get}>{firstNumber}</PaginationItem>}
                            {
                                navPrevCustom && currentPage.get > offsetToHideStart + 1
                                &&
                                <li onClick={() => (currentPage.get - navPrevCustom.steps >= firstNumber) ? currentPage.set(currentPage.get - navPrevCustom.steps) : currentPage.set(firstNumber)} className={classNames?.navPrevCustom ? classNames.navPrevCustom : undefined}>{navPrevCustom.content}</li>
                            }
                            {
                                pageNumbers.map(pageNumber => {
                                    ...
                                    if (!numberIsDisabled && pageNumber !== currentPage.get) {
                                        ...
                                        if (showNumber) {
                                            return <PaginationItem key={pageNumber} onClick={() => currentPage.set(pageNumber)} className={classNames?.item}>{pageNumber}</PaginationItem>
                                        }
                            ...
                            }
                            {
                                ...
                                &&
                                <li onClick={() => (currentPage.get + navNextCustom.steps <= lastNumber) ? currentPage.set(currentPage.get + navNextCustom.steps) : currentPage.set(lastNumber)} className={classNames?.navNextCustom ? classNames.navNextCustom : undefined}>{navNextCustom.content}</li>
                            }
                            {
                                showLastNumberAlways && (lastNumber !== firstNumber)
                                &&
                                <PaginationItem key={lastNumber} onClick={() => currentPage.set(lastNumber)} className={classNames?.item} activeClassName={classNames?.itemActive} active={lastNumber === currentPage.get}>{lastNumber}</PaginationItem>
                            }
                        </Fragment>
                        :
                        null
    ...
    return (
        <ul className={classNames?.wrapper}>
            ...
            }
            {
                ...
                &&
                <li className={classNames?.navPrev ? classNames.navPrev : undefined} onClick={() => currentPage.get > 1 && currentPage.set(currentPage.get - 1)}>
                    {navPrev}
                </li>
            }
            {
            ...
            }
            {
                ...
                &&
                <li className={classNames?.navNext ? classNames.navNext : undefined} onClick={() => currentPage.get < pageNumbers.length && currentPage.set(currentPage.get + 1)}>
                    {navNext}
                </li>
            }
            {
                ...
                &&
                <li className={classNames?.navEnd ? classNames.navEnd : undefined} onClick={() => currentPage.set(pageNumbers[pageNumbers.length - 1] as number)}>
                    {navEnd}
                </li>
            }
        </ul>
